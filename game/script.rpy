...
label scene1_start_jp:
    ...
    # show graduation and city
    "{rb}今日{/rb}{rt}きょう{/rt}は{rb}午前{/rb}{rt}ごぜん{/rt}７時{rt}じ{/rt}です。起{rt}お{/rt}きて、{rb}元気{/rb}{rt}げんき{/rt}。たくさんインタビューをしなきゃいけないので、いい朝{rt}あさ{/rt}ご飯{rt}ごはん{/rt}を食{rt}た{/rt}べました。{rb}牛乳{/rb}{rt}ぎゅうにゅう{/rt}も飲{rt}の{/rt}んだ。幸{rt}さち{/rt}に{rb}日本{/rb}{rt}にほん{/rt}のお{rb}土産{/rb}{rt}みやげ{/rt}を待{rt}ま{/rt}ちました。"
    "{rb}昨日{/rb}{rt}きのう{/rt}は{rb}大学{/rb}{rt}だいがく{/rt}から{rb}卒業{/rb}{rt}そつぎょう{/rt}しましたので、{rb}仕事探{/rb}{rt}しごとさが{/rt}しことをします。これは{rb}一番{/rb}{rt}いちばん{/rt}{rb}仕事{/rb}{rt}しごと{/rt}だので、マニラで働{rt}はたら{/rt}いていた方{rt}ほう{/rt}がいいです。多{rt}おお{/rt}くの中{rt}なか{/rt}にいい{rb}経験{/rb}{rt}けいけん{/rt}をもらいたいので、何{rt}なん{/rt}でも{rb}仕事{/rb}{rt}しごと{/rt}がいい。"
